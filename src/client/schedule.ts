...
export default class Schedule {
    ...
    public getReadAllowFacilities(): Promise<string[]> {
        return this.client.post(this.path, 'ScheduleGetReadAllowFacilities', []).then((res: schedule.FacilityIdsResponse) => {
    ...
    public getModifyAllowFacilities(): Promise<string[]> {
        return this.client.post(this.path, 'ScheduleGetModifyAllowFacilities', []).then((res: schedule.FacilityIdsResponse) => {
            const facilityIds: string[] = [];
            if (res.facility_id !== undefined) {
                res.facility_id.forEach(facilityId => {
                    facilityIds.push(facilityId);
    ...
    public getRemoveAllowUsers(): Promise<string[]> {
        return this.client.post(this.path, 'ScheduleGetRemoveAllowUsers', []).then((res: schedule.UserIdsResponse) => {
            const userIds: string[] = [];
            if (res.user_id !== undefined) {
                res.user_id.forEach(userId => {
                    userIds.push(userId);
